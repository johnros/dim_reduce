...
Notice the following equivalent facts: 
(i) The solution to the PCA problem only depends on the covariance $\Sigma$. 
(ii) $X$ is centered before computing the PCA. 
This means that the PCA solution is invariant to the location of the data. 

Starting from the maximal variance motivation, it is perhaps not surprising that PCA depends only on the similarities between features, as measured by their empirical covariance. 
The linearity of the target manifold was there by assumption. 
